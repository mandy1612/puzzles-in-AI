...
class Agent:
    ...
    def HillClimb(self):
        ...
        self.tree.root.cost = self.heuristic()
        currNode=self.tree.root
        children=self.tree.moves(currNode)
        counter=0
        ...
        while True:
            ...
            if len(children)!=0:
                for i in children:
                    i.cost = self.heuristic()
                if currNode.cost>children[0].cost or currNode.cost>children[1].cost:
                    if children[0].cost<=children[1].cost:
                        currNode=children[0]
                    elif children[1].cost<children[0].cost:
                        currNode=children[1]
                    children=self.tree.moves(currNode)
                else:
                    break
